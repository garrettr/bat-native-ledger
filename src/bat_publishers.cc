...
void BatPublishers::onSetPanelExcludeInternal(ledger::PUBLISHER_EXCLUDE exclude,
  ...
  std::string baseDomain = publisher_info->id;
  ledger_->SetPublisherInfo(std::move(publisher_info),
...
bool BatPublishers::isExcluded(
  const std::string& publisher_id, const ledger::PUBLISHER_EXCLUDE& excluded) {
...
bool BatPublishers::isEligibleForContribution(const ledger::PublisherInfo& info) {
  ...
  if (isExcluded(info.id, info.excluded) ||
    (!state_->allow_non_verified_ && !isVerified(info.id)))
    return false;
